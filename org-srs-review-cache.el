  ...
  (let ((compare-function (avl-tree--cmpfun tree)) (root (avl-tree--dummyroot tree)))
    (cl-multiple-value-bind (compare-function node-left node-right dir-left dir-right)
      ...
      (cl-macrolet ((node-left (&rest args) `(funcall node-left . ,args))
                    ...
                    (del-balance (&rest args) `(avl-tree--del-balance . ,args)))
        (cl-labels ((traverse (node &optional (callback callback))
                      (when node
                        (traverse (node-left node))
                        (traverse (node-right node))
                        (funcall callback (node-data node))))
                    (recurse (node branch)
                      (let ((br (node-branch node branch)))
                        (cond
                         ((null br) nil)
                         ...
                         (t
                          ...
                          (traverse (node-left br) callback)
                          (funcall callback (node-data br))
                          (setf (node-branch node branch) (node-right br))
                          (recurse node branch))))))
          (recurse root 0))))))
