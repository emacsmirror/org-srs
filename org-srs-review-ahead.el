    ...
    (cl-flet* ((org-srs-item-interval (&rest args)
               ...
               (item< (a b &aux (days-a (item-due-days a)) (days-b (item-due-days b)))
                 (if (= days-a days-b)
                   ...
                   (< days-a days-b)))
               (itemp (&aux (item (cl-multiple-value-list (org-srs-item-at-point))))
                 (save-excursion
                   (<= (item-due-days item) (seconds-days (org-srs-schedule-fuzz-interval-base (apply #'org-srs-item-interval item)))))))
