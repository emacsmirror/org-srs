...
- *Powerful* \\
  ...
  parameters.
- *Performant* \\
  Org-srs ships caching facilities that can significantly improve
  performance if you are willing to sacrifice some flexibility (such
  as creating items on the fly). With caching enabled, Org-srs can
  easily handle large review collections with 10K+ items.
- *Flexible* \\
